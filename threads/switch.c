...
.text
    ...
    .globl ThreadRoot
    .ent ThreadRoot,
    0 ThreadRoot : or fp, z, z #Clearing the frame pointer here
...
#a1-- pointer to new Thread
        .globl SWITCH
        .ent SWITCH,
    0 SWITCH : sw sp, SP(a0) #save new stack pointer sw s0, S0(a0) #save all the callee - save registers sw s1, S1(a0) sw s2, S2(a0) sw s3, S3(a0) sw s4, S4(a0) sw s5, S5(a0) sw s6, S6(a0) sw s7, S7(a0) sw fp, FP(a0) #save frame pointer sw ra, PC(a0) #save return address
...
nop /* These 2 nops are skipped because we are called
    ...
    0 nop
            call %
        l0,
    1 mov % l1, % o0 /* Using delay slot to setup argument to InitialPC */
                        call %
                    l2,
    0 nop
            ...
            ta ST_BREAKPOINT
                ...
                .globl SWITCH
                    SWITCH : save %
        sp,
    -SA(MINFRAME), % sp st % fp, [% i0] st % i0, [% i0 + I0] st % i1, [% i0 + I1] st % i2, [% i0 + I2] st % i3, [% i0 + I3] st % i4, [% i0 + I4] st % i5, [% i0 + I5] st % i7, [% i0 + I7] ta ST_FLUSH_WINDOWS nop mov % i1, % l0
                                                                                                                                                                                                                                 ld[% l0 + I0],
    % i0
        ld[% l0 + I1],
    % i1
